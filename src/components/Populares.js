...
const Populares = () => {
  ...
  // pone siempre un valor inicial en tu estado: en este caso, 1. asi te ahorras el useeffect que no es necesario en este caso
  const [cantidadPaginas, setCantidadPaginas] = useState();

  const obtenerPeliculas = (pagina) => {
    fetch(
      `${baseUrlApi}/movie/popular?api_key=${apiKey}&languaje=es-ES&page=${pagina}`
    )
      .then((res) => res.json())
      .then((data) => {
